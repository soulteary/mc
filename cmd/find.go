...
func stringsReplace(ctx context.Context, args string, fileContent contentMessage) string {
	...
	if strings.Contains(str, "{url}") {
		shareURL := getShareURL(ctx, fileContent.Key)
		str = strings.ReplaceAll(str, "{url}", shareURL)
		str = strings.ReplaceAll(str, `{"url"}`, strconv.Quote(shareURL))
	} else if strings.Contains(str, `{"url"}`) {
